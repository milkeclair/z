...
z.t.describe "z.git.commit._msg.build"; {
  z.t.context "tag, messageを渡した場合"; {
    z.t.it "tagとmessageを組み合わせたコミットメッセージを返す"; {
      z.git.commit._msg.build tag="feat" message="add new feature"
  ...
  z.t.context "tag, message, ticketを渡した場合"; {
    z.t.it "tag, ticket, messageを組み合わせたコミットメッセージを返す"; {
      z.git.commit._msg.build tag="fix" message="fix bug" ticket="feat-123"
  ...
  z.t.context "tag, message, cycle"; {
    z.t.it "tag, cycle, messageを組み合わせたコミットメッセージを返す"; {
      z.git.commit._msg.build tag="refactor" message="refactor code" cycle="green"
